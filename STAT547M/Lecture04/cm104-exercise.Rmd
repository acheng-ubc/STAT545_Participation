...
```
```{r}
map2(a, b, ~ .x * .y) # ".x" is the function variable by convention
```
```{r}
map2(a, b, `*`)
```
...
```{r}
map2(name, commute, function(t, s) str_c(t, " takes ", s, " minutes to get to work."))
```
...
```{r}
got_alias %>% 
  {setNames(.$aliases, .$name)} %>% # curly braces to prevent got_alias from being piped merely as the first argument of setNames()
  `[[`("Melisandre") 
```
