...
1. Mostly [stat545.com: character data](http://stat545.com/block028_character-data.html)
    - See the ["Resources" section](http://stat545.com/block028_character-data.html#resources) for a more coSmprehensive listing of resources based on the character problem you're facing. 
...
test_that("folder names are length 5.", {
    expect_true(all((str_length(folders) == 5)), TRUE)
})
```
...
```{r}
(contents <- str_c("#STAT547M Participation\n\n##Lecture ", num))
...
```{r}
str_view_all_match(str_to_lower(countries), pattern = "[^aieou ]{3}")
```
...
```{r}
str_view_all_match(countries, "([o|c])\\1")
```