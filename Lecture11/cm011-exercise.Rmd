...
```{r}
a * 1:3 # recycling = take shorter vector and duplicate until you reach length of the longer vector
```
...
```{r}
a > 0 & a < 5
a > 0 && a < 5 # this is weird implementation by R, what may be more useful is any() or all() functions:
any(a > 0 & a < 5)
```
