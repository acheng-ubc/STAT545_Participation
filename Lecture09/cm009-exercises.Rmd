...
```{r}
write_csv(iris, path = "C:/Users/user/Desktop/git_docs/STAT545_Participation/Lecture09/iris.csv")
```

Write the `iris` data frame to a file delimited by a dollar sign. 

```{r}
write_delim(iris, path = "C:/Users/user/Desktop/git_docs/STAT545_Participation/Lecture09/iris$.csv", delim = "$")
```

Read the dollar-delimited `iris` data to a tibble.

```{r}
read_delim("C:/Users/user/Desktop/git_docs/STAT545_Participation/Lecture09/iris$.csv", delim="$")
```
