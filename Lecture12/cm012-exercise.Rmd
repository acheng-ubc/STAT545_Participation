...
- with `rbind()` base-R way
- with `bind_rows()` tidyverse way

Which one is more lenient? Which would you prefer?

```{r}
rbind(lotr1, lotr2) # more lenient
...
gapminder %>%
  ...
  geom_bar() +
  scale_x_discrete(drop=FALSE) # very useful to keep levels that do not have any data after filtering
```
...
gap_big_north$country %>% 
  fct_recode("USA" = "United States", "CAN" = "Canada", "MEX" = "Mexico") %>% 
  levels()
```
...
gap_africa %>% 
    mutate(country = fct_lump(country, w=lifeExp, n=4)) %>% 
